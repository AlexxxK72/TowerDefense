...
public class GameScreen implements Screen {
    ...
    public void setTurret(int type) {
        if (playerInfo.isMoneyEnough(turretEmitter.getTurretCost(type, 0))) {
            playerInfo.decreaseMoney(turretEmitter.getTurretCost(type, 0));
            turretEmitter.setTurret(type, 0, selectedCellX, selectedCellY);
        }
        groupTurretSelection.setVisible(false);
    }
    ...
    public void upTurret() {
        TurretEmitter.TurretTemplate upTemplate = turretEmitter.getTurretTemplateUp(selectedCellX, selectedCellY);
        if(upTemplate != null){
            if (playerInfo.isMoneyEnough(upTemplate.getCost())) {
                playerInfo.decreaseMoney(upTemplate.getCost());
                turretEmitter.upTurret(upTemplate, selectedCellX, selectedCellY);
            }
        }
    }
